...
left_col = [[sg.Text('Folder'), sg.In(size=(25,1), enable_events=True ,key='-FOLDER-'), sg.FolderBrowse()],
           ...
           [sg.Button('Delete')],
           [sg.Text('Rename'), sg.InputText(enable_events=True, key='-RENAME-'), sg.Submit()]]
...
while True: 
    ...
    if event == '-FOLDER-':                         # Folder name was filled in, make a list of files in the folder
        ...
        window['-FILE LIST-'].update(sorted(fnames))
    if event == 'Submit' and text_input != '':
        submit = True
        event = '-FILE LIST-'
        ...
        try:
            ...
            window['-IMAGE-'].update(data=convert_to_bytes(filename, resize=None))
            
            if text_input != '':
                print(text_input)
                newname = os.path.join(values['-FOLDER-'], f'{text_input}.jpg')
        ...
        choice = sg.popup_yes_no('Delete file permanently?', filename)
